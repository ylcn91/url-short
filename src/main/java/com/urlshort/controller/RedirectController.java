...
public class RedirectController {
    ...
    public ResponseEntity<Void> redirect(
        ...
        recordClickEventAsync(shortLink.id(), workspaceId, code, originalUrl, ipAddress, userAgent, referrer);
    ...
    /**
     * Records a click event asynchronously by publishing to Kafka.
     * <p>
     * This method runs in a separate thread to avoid blocking the redirect response.
     * Click data is published to Kafka for asynchronous processing and includes:
     * </p>
     * <ul>
     ...
     *   <li>User agent (for device type detection)</li>
     *   <li>Referrer (for traffic source analysis)</li>
     *   <li>Country/City (from CDN headers if available)</li>
     * </ul>
     *
     * <p>
     * If click recording fails (e.g., Kafka is down), the error is logged but the
     * redirect still succeeds. This ensures that analytics failures don't impact
     * user experience.
     * </p>
     *
     ...
     */
    @Async
    protected void recordClickEventAsync(Long shortLinkId, Long workspaceId, String shortCode,
        ...
        try {
            ...
            log.debug("Click event published to Kafka for linkId={}", shortLinkId);

        } catch (Exception e) {
            // Log error but don't propagate - analytics failures shouldn't break redirects
            log.error("Failed to publish click event to Kafka for linkId={}: {}",
